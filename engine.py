...
# === Маппинг типов аспектов на русский (для texts.json) ===
ASPECT_TYPE_MAP = {
...
def get_aspect_text_key(aspect_type_en):
    """Переводит тип аспекта из английского на русский для поиска в texts.json"""
        ...
        for j in range(i + 1, len(planets_data)):
            ...
            for angle, data in ASPECTS.items():
                ...
                if abs(diff - angle) <= orb:
                    ...
                    # Если текст хранится как dict, достаём значение 'text'
